...
impact (each statement a potential impact with the condition it depends on,
labelled conditional or informed by confirmed context — a label TrustTable
derives, never one the model chooses), Remediation (advisory only) and
...
**Structured finding-analysis output (`AI-08`, D-040).** The finding-analysis call does not ask for free prose. It carries a versioned output contract (`finding_analysis_v1`, `ai_boundary/finding_analysis.py`) built per request, whose JSON schema enumerates only the evidence IDs, columns, confirmed-context fields and numeric-fact names actually supplied; `llama.cpp` receives it as a `response_format`. The response is then validated *role by role*, and the validator — not the schema — is the acceptance authority: the four roles are an explanation, potential business-impact statements (each stating the condition it depends on), advisory remediation steps, and one proposed validation rule. The model chooses no label for an impact statement: it has no `basis` field, and a response that supplies one is rejected as an unsupported control field. TrustTable derives the displayed basis itself — `confirmed_context` when a statement cites a context field that was actually sent (only user-confirmed or corrected fields of a finalized context ever are), `assumption` otherwise — and there is deliberately no `evidence` basis, because deterministic evidence establishes what was found in the data, not what it costs a business. In addition to the checks above it verifies that every number in prose is grounded in the supplied evidence or confirmed context, that an *explanation* (an assertion about the finding) uses no consequence term (loss, penalty, regulatory, customer, …) the evidence or context does not itself contain, and that remediation and rule text never claim data was changed or a rule activated. Only user-confirmed or corrected context, and only once finalized, is ever sent as `confirmed_context`. Provider-bound evidence is an allow-list of computed facts (`ai_boundary/prompt.py::serialize_evidence_for_provider`): numbers, booleans and closed-vocabulary tokens are forwarded and every dataset-derived string in a payload is withheld (failing closed for fields future detectors add); every provider-bound envelope also carries neutral evidence aliases instead of canonical evidence ids — which can embed cell content — because `build_prompt_envelope` applies them for every route (finding analysis and context inference alike). Column names remain, as data in the untrusted-data payload, because a model cannot explain a finding without naming its column (D-040). Provider and model identity leave the backend only as derived labels and a sanitized identifier (`ai_provider/display.py`), never as a filesystem path.

**What the `confirmed_context` slot carries (D-042).** The envelope's `confirmed_context` slot carries dataset context *together with each field's own `confirmation_state`* (`confirmed`, `corrected`, `inferred` or `unknown`); the slot's name comes from the domain model, and it is the state on every field, not the slot name, that says how far a value can be trusted. The two AI calls that use the slot differ on purpose. The *context-inference* call sends every context field, inferred and unknown ones included and labelled as such, because its whole purpose is to propose a value for a field that is still unconfirmed. The *finding-analysis* call sends only user-confirmed or corrected fields of a finalized context, so inferred context is never presented to that call at all. An AI-sourced context result is only ever a proposed hypothesis (a fixed confidence, provenance `ai_interpretation`); it is not marked confirmed until the user confirms or corrects it. Context is untrusted input in both calls regardless of state. Renaming or splitting the slot remains an open option, deferred so that what a model receives is not changed before the local-AI qualification measures it.
