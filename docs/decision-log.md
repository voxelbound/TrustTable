...
3. **Role-aware validation** (each a distinct, tested `RejectionReason`): schema, types, bounds and enums; provenance; evidence and column grounding at top level and per statement; a context field may be cited only if it was actually sent; an impact statement's declared basis must be supported (evidence needs evidence ids, context needs sent context fields, an assumption needs its condition and nothing else); an explanation or an evidence/context-backed statement may not use a consequence term (loss, penalty, regulatory, customer, revenue, …) that the supplied evidence or confirmed context does not itself contain — a consequence must be labelled a conditional assumption instead; every number in prose must appear in the evidence, confirmed context or known numeric facts (with a `0`/`1`/`100` allowance only inside a proposed rule's description); remediation and rule text may not claim data was or will automatically be changed or that a rule was applied or activated. `EVAL-AI-01`'s claim screen (`D-039`) still runs over every text field as defense in depth. For advice text (remediation, rule description) two narrow adjustments apply, because the shared screen cannot tell the imperative verb in "correct them at the source" from the adjective in "the data is correct", nor a request ("Verify that the values are valid") from an assertion: an imperative "correct"/"clean" followed by an object is a verb, and a sentence beginning with a directive verb plus "that"/"whether"/"if" is exempt from the two whole-dataset quality families only. Assertions (explanation, impact, assumption) get the full screen unchanged.
4. **Advisory only, by construction.** Business impact is never displayed as fact (basis is always shown). Remediation never mutates uploaded or source data and never claims to. A proposed rule is text with `status: "proposed"` — no rule engine, no execution, no activation, no export; `RULE-01`/`RULE-02` remain `v0.3`.
