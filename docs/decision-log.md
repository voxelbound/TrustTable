...
**Fixture/validator numeric-grounding defect found and fixed (2026-09-15, `WP-045`, phase-support work for the open "hands-on AI-06 evaluation" phase):** a same-session read-only diagnosis found that `ai_benchmark.fixtures._make_task` hardcoded `known_numeric_facts={}` for every one of the harness's 6 fixtures, so `ai_boundary.validation.validate_model_output` rejected *any* `numeric_claims` key with `unknown_numeric_claim` regardless of correctness — a harness defect, not a genuine model-quality signal. This was corroborated by a real hands-on run the same day against a local Qwen3.5-4B-Q4_K_M/llama.cpp candidate: its `finding_explanation` attempt was rejected 0/1 for exactly this structural reason. `WP-045` fixes derivation: `known_numeric_facts` is now built from each fixture's real `Evidence.structured_payload` numeric (`int`/`float`, non-`bool`) fields, aggregated by flat field name (no `evidence_id` namespacing); a field name with one consistent value across all included evidence is exposed, a field name with conflicting values across evidence items is omitted entirely (fails closed, never last-write-wins). No change to `validate_model_output`, the model-facing `numeric_claims` schema, `runner.py`, `metrics.py`, `persistence.py`, or any adapter. **This fix does not itself constitute a hands-on evaluation run** — the "hands-on AI-06 evaluation" phase remains open; any prior rejection observed before this fix must not be treated as a real model-quality finding, and a fresh rerun against the corrected fixtures is separate, later, human-initiated work.
