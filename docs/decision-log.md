...
**Sequencing amendment (2026-09-19, appended — see `D-040`):** `AI-08` (grounded AI analysis, recommendations and deployable local-AI experience) is inserted into the canonical order immediately after `EVAL-AI-01` and before `REL-02`, by explicit product direction: before `v0.2` is release-qualified the AI-assisted finding experience must be complete and deployable. Updated order for the remaining items: `EVAL-AI-01` (complete) → **`AI-08`** → `REL-02`. It does not reopen any decision this entry or `D-034`/`D-035` already settled, and it does not start any `v0.3` item.
...
## D-040 — Structured finding analysis: a constrained four-role output contract, advisory-only recommendations, path-free provenance and a first-class Linux install path

**Decision:**

1. **One structured call per finding.** The finding-analysis AI call (`AIOperation.FINDING_EXPLANATION`, enumeration unchanged) uses a versioned, closed output contract `finding_analysis_v1` (`ai_boundary/finding_analysis.py`) instead of one free-prose narrative. Its four **semantic roles** are validated separately: an *explanation*; 1–3 *business-impact* statements, each labelled `evidence`, `confirmed_context` or `assumption` (a conditional implication whose condition is stated); 1–3 advisory *remediation* steps; and one *proposed validation rule* whose type is drawn from the closed set in `docs/product-requirements.md` §13. There is no severity, score or finding field: any extra key is an unsupported control field. A single call keeps CPU prefill to one pass.
2. **The contract is built per request** and carried on `ProviderRequest.output_contract` (`ai_boundary/output_contract.py`; requests without one behave exactly as before, so context inference and the benchmark harness are unchanged). Its JSON schema enumerates only the evidence ids, columns, context fields and numeric-fact names actually supplied, and `LlamaCppProvider` sends it to `llama-server` as a `response_format` (grammar-constrained decoding), with a bounded output-token budget. A server that rejects `response_format` (HTTP 400/422/501) is retried once without it — only the decoding constraint is lost. **The validator, not the schema, is the sole acceptance authority**: a provider that ignores the schema is held to the same rules.
3. **Role-aware validation** (each a distinct, tested `RejectionReason`): schema, types, bounds and enums; provenance; evidence and column grounding at top level and per statement; a context field may be cited only if it was actually sent; every impact statement must state the condition under which it would hold, and a context field it cites must be one that was actually sent (the model chooses no basis — see the correction below); an *explanation* may not use a consequence term (loss, penalty, regulatory, customer, revenue, …) that the supplied evidence or confirmed context does not itself contain; every number in prose must appear in the evidence, confirmed context or known numeric facts (with a `0`/`1`/`100` allowance only inside a proposed rule's description); remediation and rule text may not claim data was or will automatically be changed or that a rule was applied or activated. `EVAL-AI-01`'s claim screen (`D-039`) still runs over every text field as defense in depth. For advice text (remediation, rule description) two narrow adjustments apply, because the shared screen cannot tell the imperative verb in "correct them at the source" from the adjective in "the data is correct", nor a request ("Verify that the values are valid") from an assertion: an imperative "correct"/"clean" followed by an object is a verb, and a sentence beginning with a directive verb plus "that"/"whether"/"if" is exempt from the two whole-dataset quality families only. Assertions (explanation, impact, assumption) get the full screen unchanged.
4. **Advisory only, by construction.** Business impact is never displayed as fact, and **no model can award its own prose an established-fact label** (correction below). Remediation never mutates uploaded or source data and never claims to. A proposed rule is text with `status: "proposed"` — no rule engine, no execution, no activation, no export; `RULE-01`/`RULE-02` remain `v0.3`.
5. **Deterministic built-in guidance** (`explanation/guidance.py`): a static, detector-keyed table for all 13 detectors gives the same four sections when AI is disabled, rejected or failing. Every template impact statement is a labelled assumption (a template cannot know a dataset's business use); remediation says changes belong in the source system.
6. **Context grounding.** Only fields the user *confirmed or corrected*, and only once the analysis is *finalized*, are sent as `confirmed_context`. Inferred and unknown fields are never sent, so an inferred value cannot reach the model — or the user — labelled as confirmed. This tightens `D-037`'s finalized-context path, which previously sent every field (including inferred ones) under a "confirmed" label; `confirmed_context_sent_to_model` is now true only when at least one confirmed field was sent.
7. **No cell value reaches a provider — an allow-list, not a block-list.** An independent review of this decision found that `WP-065` r4's redaction (a block-list keyed on one field of one evidence type) left other detectors' raw strings exposed — for example `distinct_casings`, the raw cell strings that differ only in casing — and an end-to-end test then found a second vector: a canonical **evidence id** can itself embed the normalized cell value. Both are closed at the single serialization seam (`ai_boundary/prompt.py::serialize_evidence_for_provider`): a payload field is forwarded only if it is a number, boolean or `None` (strings only for the closed-vocabulary keys `matched_pattern_categories` and `reference_date`, and only as a bare token), and a single dataset-derived string anywhere inside a field withholds the whole field. A nested mapping's *keys* are strings too (a value-count table is keyed by the values counted, and a bare-token key such as `acme` looks exactly like a detector-authored one), so keys pass the same gate: a mapping is withheld whole unless its field is one of those two closed-vocabulary keys. This fails closed: a string field a future detector adds stays invisible until deliberately allow-listed. A second review then showed the neutralization must not be per-route (the Context route also calls the provider, with the whole analysis's evidence), so **every provider-bound envelope shows neutral evidence aliases** (`evidence_1`, `evidence_2`, …) instead of canonical ids: the aliasing lives in `build_prompt_envelope`, the one constructor every route uses, and a structural test pins that no other production code builds an envelope around it. A caller that needs canonical ids back (finding analysis, when an accepted answer becomes the domain object) keeps its original evidence and maps by position. The one direct `PromptEnvelope(...)` construction outside that constructor is the benchmark harness's fixture builder — evaluation tooling over the committed synthetic demo dataset, not a route — and is disclosed here rather than covered. Evidence also carries its column keys and affected-row count, because the model is held to real columns and to numbers matching supplied counts; row numbers and row content are never added. The validator grounds numbers and terms only in this provider-visible view, so content that was withheld cannot count as "supplied". This supersedes the `WP-065` r4 block-list (whose `D-038` note is otherwise unchanged). **Stated boundary:** *column names* — untrusted dataset metadata — are still sent (in `display_safe_summary` and `affected_columns`), as data inside the untrusted-data payload, never in system instructions and bounded by the column-name length limit, because a model cannot explain a finding without naming its column; model output is validated so an instruction hidden in a header cannot alter a finding, evidence, severity or score. Neutralizing instruction-like headers before they reach a model is not part of this decision.
8. **Path-free provenance** (`ai_provider/display.py`). `LLM_MODEL` is operator text and is often a file path. The raw value is still what the provider sends to `llama-server`, but the API returns only a sanitized identifier (final path segment, bounded, control characters, drive prefixes, UNC/POSIX/tilde/URL forms and credentials removed) plus derived labels (`Local AI`, `llama.cpp`, `Qwen3.5 4B`, quantization `Q4_K_M`). The UI renders only those labels. A manually used 9B model is labelled truthfully as 9B; the documented baseline stays Qwen3.5-4B-Q4_K_M on `llama.cpp` (`D-034`/`D-035`).
9. **Linux install path.** `docs/installation-linux.md` is linked from the README quick start. `docker-compose.yml` maps `host.docker.internal` to the host gateway for the backend, because Docker Engine on Linux does not define that name and the documented default `LLM_BASE_URL` otherwise cannot resolve. `llama-server` stays a host process (`D-036`).

**Basis:** the AI integration worked but the product was incomplete: three Finding Detail sections were placeholders; the UI printed an absolute host model path; `EVAL-AI-01` showed that screening free prose lexically has open-ended coverage (the durable direction recorded there was constrained, structured output); and Linux/local-AI setup was not discoverable, with the documented default not resolving on Linux Docker. The human owner directed structured/constrained output, four grounded sections, advisory-only recommendations and first-class Linux documentation, and delegated every implementation choice.

**Alternatives considered:** (a) one call per section — simpler per-call prompts but four prefills on CPU and four failure modes; (b) extending the free-prose narrative and the claim screen — the open-ended pattern `D-039`'s own review history argues against; (c) a fixed template with the model choosing only enumerated values — the strongest guarantee, but too little value for a business user (no natural-language impact or advice); (d) a containerized `llama.cpp` service in Compose — attractive for GHCR-only hosts but not verifiable here, so it is left to `REL-02` rather than documented as supported; (e) publishing images now — a protected release action, carried to `REL-02`.

**Residual limits, stated plainly.** The structure narrows the attack surface to bounded free-text fields, and grounding rules constrain what those may cite, but the free-text fields themselves remain lexically screened: a creative paraphrase can still evade the claim screen, and a claim comma-spliced onto a directive sentence can evade the advice-text exemption (both asserted in tests). The consequence-term rule (now applied to the explanation only) and the number rule are bounded closed lists and deliberately biased toward rejecting; a false rejection degrades safely to the built-in guidance. Deterministic authority — no output field can remove a finding or change a score — remains the guarantee. How reliably a small local model fills the schema is an empirical property, not asserted here: it is a `REL-02` local-AI qualification item. The durable concern behind moving to structured output (a guarantee that does not depend on enumerating phrasings) is therefore addressed for the finding-analysis surface to the degree above, not eliminated; context inference still returns one narrative under the generic contract.

**Correction (independent semantic review, same package): the model no longer labels its own business-impact prose.** The first cut let the model choose each impact statement's `basis` (`evidence` / `confirmed_context` / `assumption`) and used a closed list of consequence stems to police the two stronger labels. An independent review showed the flaw is not in the list's coverage but in the design: a schema-valid answer such as "This will cause the company to lose money and damage its reputation.", citing real evidence and labelled `evidence`, contains none of the listed stems and was shown under a green "Evidence-backed" badge — an invented consequence presented as established. Widening the list would repeat the pattern `D-039` and `FUP-011` already show does not converge. The structural fix removes the model's authority instead: (a) `business_impact` items carry no `basis` field — a response that supplies one is rejected as an unsupported control field, and the JSON schema has no such property; (b) every statement must state the condition under which it would hold (`assumption`, non-empty); (c) TrustTable derives the presented basis from the only thing it can establish — `confirmed_context` when a statement cites a context field that was actually sent (only user-confirmed or corrected fields of a finalized context ever are), otherwise `assumption` — and shows the former as *informed by* that context, never as the consequence being confirmed; (d) there is **no `evidence` basis at all**, because the deterministic evidence establishes what was found in the data, not what it costs a business, so no wording can produce an "Evidence-backed" badge. The consequence-term list therefore no longer gates impact statements (it never decided authority); it still screens the explanation, which asserts rather than speculates. This is an intent-preserving correction under `AI-08`'s own authority, not a new scope: the four sections, the structured contract, the fallback, the trust-boundary allow-list and aliases and everything else in this decision are unchanged.
...
4. **Publishing is gated by a human act, once.** The workflow runs only for version tags (and manual dispatch, which builds without publishing); the registry login and the push are conditional on the event being a *push* of a tag on its *first attempt* (`github.event_name == 'push' && github.ref_type == 'tag' && github.run_attempt == 1`), so a manual run started from a tag, or a re-run, never publishes and a published version is never overwritten (an independent review found the first cut gated on the ref type alone, which published on a manual run from a tag); the anonymous-pull verification job has no attempt condition so it can be re-run after a package's visibility changes; `packages: write` is granted only to the publish job (the workflow default is read-only); ref names reach scripts only through the environment. A tag must be `vMAJOR.MINOR.PATCH` and equal the backend package version, so the version a running backend reports is the tag it was installed from. Provenance and SBOM attestations are off: SBOM, container scanning and signing stay with the later security and release-artifact work (`DEC-002`'s deferral is unchanged).
