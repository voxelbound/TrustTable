...
7. **Evidence is serialized with its columns and affected-row count** (`ai_boundary/prompt.py`), because the model is later held to real column keys and to numbers that match supplied counts. Row numbers and row content are not added; the raw prompt-injection excerpt redaction (`WP-065` r4) is unchanged.
