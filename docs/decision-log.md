...
## D-028 — Excel/XLSX pragmatic scope (milestone TBD)
...
**Explicitly deferred, not required unless a very cheap implementation emerges:** a formula recalculation engine, full dependency graphs, cross-sheet formula tracing, named-range dependency analysis, deep lookup-chain investigation, or Excel-compatible calculation semantics. These remain candidate future investigation features, not committed scope.

**Correction (2026-09-13, design review):** this entry was originally titled "v0.2 Excel/XLSX scope." That was inconsistent with the rest of the roadmap: `docs/implementation-backlog.md` places `ING-03` under the **Production completion / v1.0** group, and `docs/release-plan.md`'s own `v0.2` bullet list does not mention Excel/XLSX at all. The decision above is retitled to be milestone-agnostic pragmatic guidance for whenever `ING-03` is actually built, rather than asserting a scope inclusion nothing else in the roadmap reflects. **Whether XLSX support should actually be pulled into `v0.2` is an open, human-owned roadmap question, not decided here either way** — see `project-ops/changes/CHG-002-v02-local-ai-design-outcome.md`.
