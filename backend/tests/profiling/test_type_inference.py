...
Covers this package's acceptance criteria AC-01..AC-16 (r3): one positive
case per `InferredColumnType` value, boundary cases (single-value columns,
the exact 0.5 categorical ratio boundary, a large fully-unique free-text
column proving the disclosed IDENTIFIER limitation, a one-duplicate column
proving the zero-tolerance boundary), partial-shape "mixed" columns,
null/distinct-count correctness, and a real-file integration-flavored
check against the committed `demo-data/sales_demo.csv` parsed through
`ING-02`'s `parse_csv`.
"""
...
def test_large_fully_unique_free_text_column_also_classifies_as_identifier() -> None:
    # Disclosed r3 limitation, proven rather than left implicit: PROF-02's
    # IDENTIFIER only asserts "zero duplicates observed" — it cannot (and
    # per docs/domain-model.md §8 / docs/product-requirements.md §10, is
    # not expected to) distinguish a true identifier from a large,
    # coincidentally fully-unique free-form text column. That refinement
    # belongs to PROF-03's "likely identifier" text metric and later
    # DatasetContext.candidate_keys confirmation, not to this layer.
    columns = make_columns("complaint_text")
    values = [f"Customer complaint number {i} about a totally unrelated issue" for i in range(50)]
...
def test_one_duplicate_value_column_is_not_classified_as_identifier() -> None:
    # r3 replaces r2's >=95%-distinct tolerance with a literal zero-
    # duplicate-tolerance rule: even a single duplicate value excludes a
    # column from IDENTIFIER (see the real-file AC-12 case below for the
    # motivating example, order_id with one injected duplicate).
    columns = make_columns("order_id")
    rows = make_rows(("ORD-0001",), ("ORD-0002",), ("ORD-0001",))
...
def test_low_cardinality_column_classifies_as_categorical() -> None:
    ...
    rows = make_rows(
        ("North",), ("South",), ("North",), ("East",), ("North",), ("South",), ("East",), ("North",)
    )
...
def test_exact_half_distinct_ratio_boundary_classifies_as_categorical() -> None:
    # distinct_count / total == 0.5 exactly — the CATEGORICAL boundary is
    # inclusive.
...
# ---------------------------------------------------------------------------
# AC-12 (r3, revised): real committed demo dataset
# ---------------------------------------------------------------------------
...
def test_infers_expected_types_for_committed_demo_csv() -> None:
    ...
    # r3 change from r2: order_id has one DEMO-01-injected duplicate value
    # (298/299 distinct, ratio 0.997), which the new zero-duplicate-
    # tolerance IDENTIFIER rule excludes — it now classifies as TEXT. This
    # is an intended, disclosed consequence of the r3 design, not a
    # regression (see the work package's Provenance assumption 4).
    assert by_name["order_id"].inferred_type is InferredColumnType.TEXT
