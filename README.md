...
`GET .../findings/{finding_id}`, `GET .../findings/{finding_id}/evidence`,
`POST .../cancel`) and through the real frontend: open the running app
and choose "Try the sales demo" on the Start screen to see a progress
view, a trust-assessment Overview, a filterable Findings list, and — by
selecting any finding — a Finding detail screen with its evidence, or,
for the prompt-injection detector's own finding, a dedicated warning
presentation explaining what was detected, whether it was sent to a
model, and what protections apply. Generic file upload (arbitrary
CSV/Excel analysis creation, as opposed to the bundled demo dataset) is
not yet exposed — the Start screen's upload control is present but
disabled; this remains the one open `UI-01` item.
