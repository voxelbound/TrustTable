...
prompt-injection-warning screen are not built yet in the frontend
(`UI-01` remains partial) — the backend endpoints they need now exist
(`GET .../findings/{finding_id}` and `.../evidence`), so this is a
frontend-only follow-on, not a backend gap. Generic file upload
(arbitrary CSV/Excel analysis creation, as opposed to the bundled demo
dataset) is also not yet exposed — the Start screen's upload control is
