...
Local AI beta (v0.2) work is well underway. The AI provider interface, its two non-real providers (disabled, mock), a local-AI benchmark harness, and the hands-on multi-round evaluation it enabled are all complete. Qwen3.5-4B-Q4_K_M and `llama.cpp` are the selected baseline model and runtime (`docs/decision-log.md` D-034/D-035); the real local-inference provider (`AI-03`, `ai_provider/llama_cpp.py`) is now implemented and registered (`llm_provider="llama_cpp"`), though `Settings.llm_provider` still defaults to `"disabled"`. A hands-on setup guide for running the runtime and baseline model (`AI-04`, see [`docs/local-development.md`](docs/local-development.md)) is also complete. `CTX-01` (deterministic context hypotheses) is now implemented: a framework-independent, AI-free heuristics module infers likely candidate keys, business dates, measures, dimensions, row grain, primary entity, and a narrow probable-domain guess directly from already-computed profiling facts. `CTX-02` (validated AI context inference) is also implemented as an enabling slice: a module that calls a real `AIProvider`, validates its response through the existing trust boundary, and folds an accepted result back into the deterministic context above — proven against the real disabled/mock providers, but **not yet wired into any live analysis** (no route/UI consumes `CTX-01`/`CTX-02`/`CTX-03` yet, and `Analysis.security_exposure` is unchanged). `CTX-03` (guided questions) is also implemented, generation only: a deterministic module produces up to five business-language clarification questions from whatever `CTX-01`/`CTX-02` context remains unresolved. `API-02` (Context API) is now implemented as a service-layer enabling slice: get/confirm/answer/finalize context-confirmation logic (including `ClarificationAnswer`, `docs/domain-model.md` §11, for the first time) is available as an additive layer over an already-completed analysis — **the analysis pipeline itself is deliberately unchanged** (no new `AnalysisState`, no pause for confirmation) to avoid regressing the already-shipped `v0.1`/`v0.1.1` UI; no FastAPI route exists yet. The accelerated/GPU hardware-tier default remains explicitly deferred, non-blocking. `llama-server`'s runtime profile is now hardened (`AI-07`): its built-in Web UI is disabled and it runs on its own dedicated port (`8081`), separate from TrustTable's own frontend port (`8080`) — TrustTable remains the sole user-facing application. `AI-05` (grounded finding explanations) is now implemented as an enabling slice: a deterministic, always-available explanation (closing a `docs/product-requirements.md` §5.7 gap that had no prior implementation) plus a validated AI-grounded counterpart, neither yet wired into any live analysis. A two-phase context/AI architecture decision (`docs/decision-log.md` D-037) is now recorded: the deterministic pipeline stays unchanged and unattended; context confirmation and AI interpretation are to become a second, optional, additive enrichment phase over an already-completed analysis, exposed through real API routes rather than left as another disconnected enabling slice. `UI-02` slice 1 is now implemented on top of that decision: a real `GET .../findings/{finding_id}/explanation` route (deterministic explanation always, validated AI explanation when a provider is configured, graceful fallback on rejection) and a new "Explanation" section in the shipped Finding Detail screen showing the narrative, provenance, and model location. `UI-02` slice 2 (the Context screen — editable fields, guided questions, confirm/finalize interaction), the actual Context API HTTP routes for that flow, live-pipeline wiring for `CTX-01`/`CTX-02`/`CTX-03`, and `EVAL-AI-01`/`REL-02` remain open.
