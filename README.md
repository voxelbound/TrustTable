...
> A GitHub-and-GHCR-only install (no PyPI, npm or container base-image access) is supported through the **published `v0.2.0` images**: `docker-compose.release.yml` pulls them and builds nothing, and this was verified on a clean Linux host with AI off ([`docs/release-images.md`](docs/release-images.md)). `docker compose up --build` above builds the images from source and does need the base-image registry, PyPI and npm. The guide states exactly what was and was not verified.
...
**Current milestone: v0.2 — Local AI beta (released as `v0.2.0`).**
