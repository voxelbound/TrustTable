...
TrustTable has completed its repository foundation (**FND-01**), typed configuration (**FND-02**), continuous integration (**FND-03**), common API errors (**FND-04**), the OpenAPI frontend contract pipeline (**FND-05**), the synthetic sales generator (**DEMO-01**), the parsing contracts (**ING-01**), the secure CSV parser (**ING-02**), the profiling schemas (**PROF-01**), type inference (**PROF-02**), core profiling (**PROF-03**), the detector interface (**DET-01**), the initial detector set (**DET-02** — all 12 detectors: duplicate rows, empty columns, excessive missing values, missing likely identifiers, inconsistent capitalization, leading/trailing whitespace, future dates, negative measures, invalid percentages, line-total mismatch, suspiciously constant columns, and extreme outliers), the LLM input/output trust boundaries (**SEC-02** — the untrusted-data envelope, safe prompt builder, and model-output validator; no real LLM provider is wired in yet), the prompt-injection risk detector (**DET-SEC-01** — `security.possible_llm_prompt_injection`, the required AI-processing-security detector, completing the detector catalogue at 13/13), deterministic risk scoring (**RISK-01** — per-finding priority scores and a four-label dataset trust assessment, computed with no AI/LLM input path at all), the initial analysis API (**API-01** — the in-memory analysis-orchestration engine composing parsing/profiling/detection/scoring into one create-then-run pipeline over the bundled demo dataset, exposed over real `/api/v1` HTTP routes: `POST /demo/sales`, `GET /analyses/{id}`, `GET /analyses/{id}/status`, `GET /analyses/{id}/profile`, `GET /analyses/{id}/findings`, `GET /analyses/{id}/findings/{finding_id}`, `GET /analyses/{id}/findings/{finding_id}/evidence`, `POST /analyses/{id}/cancel` — the last two added by a backend extension closing the concrete gap `UI-01` disclosed below), the basic investigation UI (**UI-01**, complete — a real Start screen with a working sales-demo action and a generic CSV upload control, a named-stage progress view, an Overview screen (trust assessment, top findings, dataset summary), a Findings list with severity/category/search filtering, a Finding detail screen (observation, evidence, representative examples, technical metadata, and honestly-disclosed not-yet-available sections for business impact/remediation/validation rule/review controls, which no package computes yet), and a dedicated prompt-injection-warning presentation for `ai_processing_security` findings — all reading the running backend API), Docker Compose production-configuration hardening (**REL-01**, scoped — both services now restart automatically after an unexpected exit; SBOM generation and container image scanning are explicitly deferred to **SEC-01**, a later backlog item, not part of this package), and generic file-upload analysis creation (**API-01**, extending — `POST /api/v1/analyses`, CSV only; extension/size validation and filename sanitization; `.xlsx` remains **ING-03**, a later backlog item). Every bullet in `docs/release-plan.md`'s **v0.1 — deterministic vertical slice** checklist is now implemented; formal v0.1 release qualification against `docs/testing-strategy.md`'s release gates remains a separate, later checkpoint this note does not itself claim.

Local AI beta (v0.2) work has begun with the AI provider interface (**AI-01**, interface-only — a framework-independent contract package defining the six model-call operations, request/response/health-check shapes built on **SEC-02**'s existing untrusted-data envelope and output validator, a provider-error hierarchy, and the `AIProvider` protocol every future provider implements against; no real provider is wired in yet — that is **AI-02** (disabled/mock) and **AI-03** (Ollama), later backlog items).

The product, domain model, API boundaries, detector framework, frontend architecture, testing strategy, threat model, release plan, and implementation backlog were defined before production implementation began. Every v0.1 deterministic-vertical-slice item in `docs/release-plan.md` is now implemented and tested, as detailed in "Project status" above and `docs/implementation-backlog.md`; local AI beta (v0.2) is now underway (**AI-01**, interface-only) and later releases remain future work.
