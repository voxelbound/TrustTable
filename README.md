...
> A GitHub-and-GHCR-only install (no PyPI, npm or container base-image access) is **not supported yet**: `docker compose up --build` builds the images from source. Publishing images is a planned `v0.2` packaging requirement: the pull-only Compose file and the tag-gated publish workflow are now in the repository ([`docs/release-images.md`](docs/release-images.md)), but no image has been published yet. The guide states the current limitation exactly.
...
**Current milestone: v0.2 — Local AI beta (release preparation).**

- **Status:** every planned `v0.2` feature is implemented and the backend is versioned `0.2.0`. Nothing has been tagged or published yet. `Settings.llm_provider` still defaults to `"disabled"`, so every AI capability degrades gracefully to the built-in guidance.
- **Remaining for `v0.2`:** `REL-02` only — the protected publish of the versioned images, one clean-host pull/install/start check against them, and the updated install guide (`docs/decision-log.md` D-041). The pull-only Compose file and the publish workflow are defined but have not been used against a registry.
...
- **REL-02 (step 1 of 3, does not complete `REL-02`)** — a pull-only Compose file (`docker-compose.release.yml`) and a tag-gated GHCR publish workflow, defined and tested but **not published or verified**; see [`docs/release-images.md`](docs/release-images.md) (`docs/decision-log.md` D-041).
